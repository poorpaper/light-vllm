...
step 仍处于正常范围。参照 vLLM 的启动期 heap freeze 生命周期，最终实现只在独立 CUDA Engine 模式的父、子进程
各自 ready 前收集并冻结启动期堆，关闭时解冻，不改变同进程和 TP 路径。

修复前六次普通服务运行的吞吐中位数为 4877.29 tok/s，最坏单请求 ITL 为 197.88 ms；修复后六次为
5014.15 tok/s 和 56.25 ms，分别改善 2.81% 和 71.57%。后者仍高于 vLLM 六次运行中的最坏 37.04 ms，后续可以
继续分析普通请求对象分配和 HTTP 事件交付，但不扩张本阶段量化实现范围。
...
- CUDA 服务 ready 前的真实执行链预热，以及独立 Engine 父子进程的启动期 GC 堆冻结和关闭清理。
