...
if [[ $# -lt 3 || $# -gt 6 ]]; then
  echo "usage: $0 ROOT MODE OUTPUT_DIR [PORT] [REQUEST_RATE] [LIGHT_TTFT_SLO_SECONDS|off]" >&2
  exit 2
fi
...
if [[ "$LIGHT_TTFT_SLO_SECONDS" != off ]]; then
  light_common+=(--max-tolerable-ttft-seconds "$LIGHT_TTFT_SLO_SECONDS")
fi

case "$MODE" in
  light-strict)
    server=("${light_common[@]}" --enable-prefix-caching)
    ;;
  light-optimistic)
    server=(
      "${light_common[@]}"
