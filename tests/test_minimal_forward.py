...
from light_vllm.contracts import ModelFactory
...
def test_state_dict_loader_round_trip(tmp_path: Path) -> None:
    ...
    restored_output = restored.forward(batch)
...
def test_components_can_be_added_without_core_changes() -> None:
    ...
    output = runner.forward(ForwardBatch(input_ids=torch.tensor([[2, 4]])))
...
def test_failed_reload_keeps_current_model() -> None:
    ...
    assert runner.generation == 1
    output = runner.forward(ForwardBatch(input_ids=torch.tensor([[1]])))
