...
TP 命令流插入额外步骤，避免与分布式执行生命周期交叉。独立 Engine 模式下，CUDA 子进程完成预热后、HTTP 父进程
完成 IPC 启动后，二者各自先收集各代对象再冻结启动期 GC 堆，让长期存活的模型、物理 KV、tokenizer、FastAPI 与
IPC 对象不参与 serving 期间的 full collection；关闭时分别解冻。这条策略不进入同进程或 TP 路径，也不改变
Engine、Scheduler 和模型契约。
